...
def message(title, content):
    ...
    message_box.setText(content)  # 设置文本内容
...
class MyApp(QMainWindow):
    ...
    def inputpath_txt(self):
        # 处理槽函数逻辑
        self.txt_path = self.ui.inputpath.text()  # 将获取到的输入路径赋值给成员变量 input_path
    ...
    def openpath(self):
        ...
        file_path = file_dialog.getExistingDirectory(self, "选择文件路径")
        self.ui.output_path.setText(file_path)
        self.inputpath_out()

    def open(self):
        file_dialog = QFileDialog()
        file_path = file_dialog.getOpenFileName(self, "选择输入文件")[0]
    ...
    def product_cloud(self, text, stop_words):
        ...
        # print(self.out_path + "/" + formatted_time + ".png")
        message("提示", "成功！")
    ...
    def run(self):
        ...
        try:
            print(self.ui.switch_stopword.isChecked())
